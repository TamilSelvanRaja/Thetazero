...
// Add this middleware configuration before defining routes
app.use(session({
  secret: 'ELeJatDoVcHYWRROkmnxdRjKQKQcoI',
  resave: false,
...
app.post('/Server/adminlogin', (req, res) => {
  ...
  db.query(checkUserQuery, [email], (checkUserErr, checkUserResults) => {
    ...
    if (password === storedPassword) {
      ...
      return res.status(200).json({ message: 'Login successful', tableName: tableName, adminId: req.session.adminId, select: select });
    } else {
...
// AdminLogout
app.get('/Server/Adminlogout', (req, res) => {
  // Destroy the session
...
app.post('/Server/memberlogin', (req, res) => {
  ...
  db.query(checkUserQuery, [phone], (checkUserErr, checkUserResults) => {
    if (checkUserErr) {
      ...
      // Phone number matched, store userId in session
      req.session.userId = checkUserResults[0].v_id;
      //new
      req.session.userPhone = checkUserResults[0].v_phone;
      req.session.userName = checkUserResults[0].v_name;
      // Phone number matched, send the user's name and other details
