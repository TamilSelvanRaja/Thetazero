...
function Main() {
  ...
  const handleExhibitorChange = useCallback((value: string[]) => {
    ...
    const promises = value.map(exhibitor => {
      ...
      .then(response => {
        const timeSlotsWithoutSeconds = response.data.exhibitorTimeSlots[exhibitor].map((timeSlot: string) => {
          const [hour, minute] = timeSlot.split(':')[0];
          return `${hour}:${minute}`;
        });
        setExhibitorTimeSlots(prevState => ({
          ...prevState,
          [exhibitor]: timeSlotsWithoutSeconds
        }));
      })
      .catch(error => {
                ...
                <TinySlider options={{
                    ...
                    <div className="h-32 px-2">
                        <div className="h-full rounded-md bg-slate-100 dark:bg-darkmode-400">
                            ...
                            Reschedule Form - Water Today Water Expo
                            </h3>
                        </div>
                    </div>
            ...
            <div className="intro-y box">
              <div className="p-5">
                ...
                <div className="flex justify-center mt-5">
                  ...
                  >
                    {isSubmitting ? "Rescheduled..." : "Reschedule"}
                  </Button>
                </div>
              </div>
            </div>
